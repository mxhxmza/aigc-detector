...
Usage:
    ...
    python app.py --checkpoint checkpoints_cifake/full.pt --port 8000
...
async def predict(image: UploadFile = File(...)) -> JSONResponse:
    ...
    scorer = get_scorer()
    with _scorer_lock:
        d = scorer.score_many_detailed([img])[0]

    p = d["p_ai_generated"]
    label, band = verdict(p)

    # A tampered image (real photo with an AI-edited region) still counts as
    # authentic for the verdict -- it was taken by a person. We only surface
    # the edit as a note when the model puts real weight on that class.
    p_tampered = d["p_tampered"]
    edited = p < 0.5 and p_tampered >= 0.35 and p_tampered >= d["p_real"]
    note = ("Looks like a real photograph with an AI-edited region. "
            "Counted as authentic, but treat the edited area with caution."
            ) if edited else ""

    return JSONResponse({
        "filename": image.filename,
        "p_ai_generated": round(p, 6),
        "p_authentic": round(d["p_authentic"], 6),
        "p_tampered": round(p_tampered, 6),
        "verdict": label,
        "band": band,
        "edited_region_suspected": edited,
        "note": note,
...
def main() -> int:
    ...
    ap.add_argument("--checkpoint", type=Path,
                    default=Path("checkpoints_cifake/full.pt"))
    ...
    if not args.checkpoint.exists():
        raise SystemExit(
            ...
            "       Train one first (see README §3.1), or pass --checkpoint."
        )
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .num span { color: var(--muted); font-size: 12px; text-transform: uppercase;
    letter-spacing: .04em; }
  .note { margin-top: 12px; padding: 10px 12px; border-radius: 10px;
...
<div class="wrap">
  ...
  <p class="sub">Upload an image. The model returns a calibrated probability that it was AI-generated. Runs locally &mdash; nothing leaves your machine.</p>
...
function show(d) {
  ...
  if (d.note) { note.textContent = d.note; note.style.display = 'block'; }
